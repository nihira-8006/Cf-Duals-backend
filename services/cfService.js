...
const watchForSolve = (duelId, handle, contestId, problemIndex, startTimeMs, onSolve) => {
    ...
    const checkStatus = async () => {
        if (!activeWatchers.has(watcherKey)) return; // Stop if watcher was cancelled
        ...
        try {
            ...
            if (response.data.status === 'OK' && response.data.result.length > 0) {
                ...
                const isTargetProblem = latestSub.problem.contestId === contestId && latestSub.problem.index === problemIndex;
                ...
                if (isTargetProblem && isAfterDuelStart) {
                    if (latestSub.verdict === 'OK') {
                        ...
                        onSolve(subTimeMs); // Trigger the win condition!
                        return;
        ...
        // Poll again in 4 seconds to avoid rate limits
